...
--fair algorithm RousseauCommit {
  ...
  process (node \in shards)
  {
    ...
    c_loop:+ while ({p \in proc_chan: (p[2] \notin accepted[self] \cup rejected[self]) /\  TTT[p[2]].dep \subseteq exist_resources} # {})
    {   
            with (m \in {p \in proc_chan: p[2] \notin accepted[self] \cup rejected[self]})
            ...
            if (active_commit # {})
            with(m \in active_commit)
            {                                                            
                if (<<"commit", m[2], "yes">> \in commit_chan){                            
                    send(accepted[self], { m[2] } );
                    (* Updates the resources for this node *)
                    resources[self] := resources[self] \cup {newres \in TTT[m[2]].new : (newres - 1) % N = (self - 1) };
    ...
    assert accepted[self] \cup rejected[self] = {t \in (DOMAIN TTT) : TTT[t].dep \subseteq exist_resources} ;    
...
c_loop(self) == /\ pc[self] = "c_loop"
                /\ IF {p \in proc_chan: (p[2] \notin accepted[self] \cup rejected[self]) /\  TTT[p[2]].dep \subseteq exist_resources} # {}
                      THEN /\ \E m \in {p \in proc_chan: p[2] \notin accepted[self] \cup rejected[self]}:
                                IF TTT[m[2]].dep \subseteq  exist_resources
                                   THEN /\ Assert(        (
                                                  LET    my_votes == {v \in votes_chan : v[4] = self /\ v[2] = m[2]}
                                                         decisions == {v[5] : v \in my_votes }
                                                          ...
                                                          LET used ==         UNION { TTT[f0].dep:
                                                            ...
                                                            IF TTT[m[2]].dep \intersect resources[self] # {}
                                                               THEN /\ Assert(\A i \in good_votes \ rejected[self] : (\A j \in accepted[self]:
                                                                               i # j => TTT[i].dep \intersect TTT[j].dep = {}), 
                                                                              "Failure of assertion at line 70, column 29.")
                                                                    /\ IF TTT[m[2]].dep \intersect used = {}
                                                                          ...
                                                                          ELSE /\ votes_chan' = (votes_chan \cup ({ <<"vote", m[2], resx, self, "no">> : resx \in TTT[m[2]].dep \intersect used}))
                                                                               /\ Assert(m[2] \notin votes[self], 
                                                                                         "Failure of assertion at line 81, column 33.")
                                                                               /\ votes' = votes
                                                               ELSE /\ TRUE
                                                                    /\ UNCHANGED << votes, 
                                                                                    votes_chan >>
                                ...
                                IF active_commit # {}
                                   THEN /\ \E m \in active_commit:
                                             /\ IF <<"commit", m[2], "yes">> \in commit_chan'
                                                   THEN /\ accepted' = [accepted EXCEPT ![self] = (accepted[self]) \cup ({ m[2] })]
                                                        /\ resources' = [resources EXCEPT ![self] = resources[self] \cup {newres \in TTT[m[2]].new : (newres - 1) % N = (self - 1) }]
                                                        /\ exist_resources' = (exist_resources \cup TTT[m[2]].new)
                                                        /\ UNCHANGED rejected
                                   ...
                                   ELSE /\ TRUE
                                        /\ UNCHANGED << exist_resources, 
                                                        ...
                                                        rejected >>
                           /\ pc' = [pc EXCEPT ![self] = "c_loop"]
                      ELSE /\ Assert(accepted[self] \cup rejected[self] = {t \in (DOMAIN TTT) : TTT[t].dep \subseteq exist_resources}, 
                                     "Failure of assertion at line 130, column 5.")
                           /\ Assert(accepted[self] # {}, 
                                     "Failure of assertion at line 133, column 5.")
...
Consensus == <>(\A self1 \in shards: \A self2 \in shards: (accepted[self1] = accepted[self2] /\ rejected[self1] = rejected[self2] /\ accepted[self1] # rejected[self2]))


=============================================================================
\* Modification History
\* Last modified Mon Apr 04 02:05:21 BST 2016 by george
