...
class Listener(Thread):
    ...
    def __init__(self, kafka_node, addr, channels):
        Thread.__init__(self)
        self.node = kafka_node
        self.daemon = True

        assert len(channels) == 1
        channel = channels[0]
        self.consumer = KafkaConsumer(channel, group_id='consensus',
                         bootstrap_servers=['%s:%s' % addr])
...
class KafkaNode(Node):
    ...
    def __init__(self, start = [], quorum=1, name = None, 
            ...
            host='localhost', port=9092, db=0):
        self.r = KafkaClient('%s:%s' % (host,port)) # redis.StrictRedis(host, port, db)
        self.shard_id = shard
        ...
        if shard is not None:
            ...
            self.client = Listener(self, addr, ['votes:%s' % self.shard_id])
        else:
            self.client = Listener(self, addr, ['votes'])
        ...
        if self.shard_map is not None:
            for i in self.shard_map:
                ...
                if within_TX(tx, b0, b1):
                    ...
                    self.r.send_messages('votes:%s' % i , msg)
        else:
            self.r.send_messages('votes' , msg)
